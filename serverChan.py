...
threading.Thread(target=updateGameThread).start()
...
class GameCentral(object):
    ...
    def cntsrv_startServ(self,port=9000):
        self.tcpServerChan = socket.socket(socket.AF_INET,socket.SOCK_STREAM)#eventlet.listen(('',port))                     
        self.tcpServerChan.bind(('',port))
        self.tcpServerChan.listen()
        ...
        def waitRoomClient():
            while 1:  
                try:
                    ...
                    conn.setblocking(0)
                    conn.settimeout(.5)                                
                    threading.Thread(target=lambda:waitRoomMsgNode(conn)).start()
                    ip,port=conn.getpeername()
                    print('CENTRAL SERVER client has connected peer is>>',ip,port)
                except Exception as e:
                    pass
                    print(e)      
        threading.Thread(target=waitRoomClient).start()
    ...
    def cntsrv_toAllclients(self,jsn):
        for key, clnt in self.clients.items():   
            ...
            threading.Thread(target=lambda:sckt_sendToOtherSocket(jsn,clnt['con'])).start()
        ...
        def cntsrv_updateAndRenderGame(game,roomID):
            ...
            for key,clnt in self.clients.items():
                if(clnt['room'] == roomID):
                    ...
                    threading.Thread(target=lambda:drawClient(clnt['name'])).start()
        
        for key,room in self.rooms.items():
            if(room['game'] != None):
                #eventlet.spawn_n(cntsrv_updateAndRenderGame,room['game'],key)
                #threading.Thread(target=lambda:cntsrv_updateAndRenderGame(room['game'],key)).start()
                cntsrv_updateAndRenderGame(room['game'],key)
                
gamecentral = GameCentral()
gamecentral.cntsrv_startServ()  
...
def makeNewDevGame(rq):
    ...
    game.addCharacter('Jon MingX 2','miming')
    game.addCharacter('Jon MingX 3','miming')    
    game.addCharacter('Jon MingX 4','miming')
    game.addCharacter('Jon MingX 5','miming')    
    game.addCharacter('Jon MingX 6','miming')    
    game.addCharacter('Jon MingX 7','miming')    
    game.addCharacter('Jon MingX 8','miming')    
